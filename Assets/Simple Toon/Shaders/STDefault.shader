Shader "Simple Toon/SToon Default"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
				half dissolve_value = tex2D(_DissolveTexture, i.uv).r; //Get how much we have to dissolve based on our dissolve texture
				...
				postCol.a = 1.;
				//if (dissolve_value>0)
				//	postCol.r = step(dissolve_value - _Amount, 0.05f);
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
				...
				postCol.a = 1.;
				if (_Amount > 0) {
					if ((dissolve_value - _Amount) < 0.05f) {
						postCol.r = 0.6 * step(dissolve_value - _Amount, 0.05f);
						postCol.g *= (1.1 - step(dissolve_value - _Amount, 0.05f));
						postCol.b *= (1.1 - step(dissolve_value - _Amount, 0.05f));
					}
				}
